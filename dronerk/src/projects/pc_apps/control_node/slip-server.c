...
int drk_send_packet(int fd, uint8_t *buf,int size, uint8_t *new_buf);
...
uint8_t buf[1024];
uint8_t send_buf[MAX_PACKET_LENGTH];
int got_connection;
int debug;
int reply_mode;
...
void server_non_blocking_rx (int fd)
{
  ...
  if(reply_mode==STATIC_CLIENT)
    ...
    n=drk_send_packet(fd,buf,n,send_buf);
//    slip_tx (fd, buf, n);
  }
}
...
int drk_send_packet(int fd, uint8_t *buf, int size, uint8_t *new_buf) {
    int i=0;
    int n, offset=0, buf_offset = 0;

    if(size<=MAX_FRAGMENT_LENGTH) {
        i=sprintf(new_buf,"$RAD:%c%c%c%c%c",packet_id,DEFAULT_TTL,1,1,0);
        memcpy(new_buf+i,buf,size);
        slip_tx(fd,new_buf,size+i);
    }
    packet_id = ((packet_id + 1) % 98) + 30;
    return 0;
